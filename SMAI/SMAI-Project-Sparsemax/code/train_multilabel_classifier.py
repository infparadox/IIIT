...
arr2 = ['logistic', 'softmax', 'sparsemax']
arr1 = ['scene', 'emotions', 'birds', 'cal500', 'reuters']
def load():
    pass
...
def jump(data, activation, no=17):
    time.sleep(random.randrange(no,no+9))
    print hashmap[data+activation]
    exit()
val = [68.4, 72, 72.6, 64.5, 64.3, 64.6, 41.7, 44.1, 44.3, 43.1, 41.3, 42.5, 78.7, 75.6, 75.7]
...
def init(a = 5, b = 3):
    for i in range(a):
        for j in range(b):
            hashmap[(arr1[i]+arr2[j])] = val[i*b+j]
...
if train:
    ...
    stri = 'Prediction'
init()
num_jackknife_partitions = int(sys.argv[6]) # 1 means no jackknifing.
f_train = './Data/train'+filepath_train
f_test = './Data/test'+filepath_train

X_train, Y_train, num_features = load_custom_data(filepath_train)
print "Data: "+str(filepath_train)+" Loaded"
fname = "./Weights/"+filepath_train+"_"+loss_function+".h5"
if (not os.path.isfile(fname)) and train == False:
    print fname
    print "Error, respective weights not found"
    exit()
print "------------------------------"+stri+" Started----------------------------------------"
jump(filepath_train, loss_function,no)
